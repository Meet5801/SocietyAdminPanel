...
import { faEdit, faTrash, faInfo, faAdd, faSpinner,faPlus } from '@fortawesome/free-solid-svg-icons';
...
const Besnu = () => {
    ...
    const UpdateForm = ({ BesnuId, BesnuItem, onClose, villages }) => {
        ...
        useEffect(() => {
            // Fetch the image
            if (BesnuItem.image) {
                axios.get(`https://expodersfour-001-site1.ltempurl.com/GetImage/${BesnuItem.image}`, { responseType: 'arraybuffer' })
                    .then(response => {
        ...
        const [formData, setFormData] = useState({
            id: BesnuItem.id || '',
            name: BesnuItem.name || '',
            lookup_village_id: BesnuItem.lookup_village_id || '', // Corrected to lookup_village_id
            death_date: BesnuItem.death_date ? BesnuItem.death_date.split('T')[0] : '', // Extract date part only
            besnu_date: BesnuItem.besnu_date ? BesnuItem.besnu_date.split('T')[0] : '', // Extract date part only
        ...
        return (
            <div className="modal show" tabIndex="-1" role="dialog" style={{ display: 'block', backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
                <div className="modal-dialog" role="document">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title">Update Besnu</h5>
                            <button type="button" className="btn-close" onClick={onClose}></button>
                        </div>
                        <div className="modal-body">
                            <form onSubmit={handleSubmit}>
