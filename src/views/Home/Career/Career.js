...
import { faEdit, faTrash, faInfo, faAdd, faSearchLocation, faSpinner,faPlus } from '@fortawesome/free-solid-svg-icons';
...
const Career = () => {
            ...
            render: (item) => {
                ...
                return (
                    <>
                        <div>
                            <img
                                src={`https://expodersfour-001-site1.ltempurl.com/GetImage/${item.career_image}`}
    ...
    const UpdateForm = ({ CareerId, CareerItem, onClose, occupation, businessCategory }) => {
        ...
        useEffect(() => {
            // Fetch the image only if it's not already loaded
            if (CareerItem.career_image && !oldImage) {
                axios.get(`https://expodersfour-001-site1.ltempurl.com/GetImage/${CareerItem.career_image}`, { responseType: 'arraybuffer' })
                    .then(response => {
        ...
        }, [CareerItem.career_image, oldImage]);
        ...
        return (
            <div className="modal show" tabIndex="-1" role="dialog" style={{ display: 'block', backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
                <div className="modal-dialog" role="document">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title">Update Career</h5>
                            <button type="button" className="btn-close" onClick={onClose}></button>
                        </div>
                        <div className="modal-body">
                            <form onSubmit={handleSubmit}>
                                ...
                                <div className="mb-3">
                                    ...
                                    <select className="form-control" name="lookup_occupation_id" value={formData.lookup_occupation_id} onChange={handleInputChange} required>
                                        <option value="">Select Occupation</option>
                                        {occupation.length > 0 && occupation.map((o) => (
