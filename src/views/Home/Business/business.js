...
import { faEdit, faTrash, faInfo, faAdd, faSpinner, faPlus } from '@fortawesome/free-solid-svg-icons'; // Changed faAdd to faPlus
...
const Business = () => {
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            ...
            const response = await axios.put(`https://expodersfour-001-site1.ltempurl.com/api/Business/Update?id=${selectedBusiness.id}`, formDataCopy, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                    Authorization: `Bearer ${token}`,
                }
...
const UpdateForm = ({ categories, cities, states, countries, formData, setFormData, onSubmit, onClose, userdata }) => {
    ...
    useEffect(() => {
            // Fetch the image
            if (formData.business_image_1) {
                axios.get(`https://expodersfour-001-site1.ltempurl.com/GetImage/${formData.business_image_1}`, { responseType: 'arraybuffer' })
                    .then(response => {
        ...
        const handleFileChange = (e) => {
            ...
            setOldImage(formData.business_image_1);
                        ...
                        <form onSubmit={onSubmit}>
                            <div className="col-md-3 d-flex justify-content-center align-items-center">
                                <div className="mb-3" style={{ width: '100px', height: '100px', border: '1px solid black', cursor: 'pointer' }}>
                                    <label className="form-label" style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }}></label>
                                    <input id="fileInput" type="file" className="form-control" name="business_image_1" onChange={handleFileChange} style={{ display: 'none' }} />
